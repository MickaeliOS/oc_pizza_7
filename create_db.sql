...
CREATE TABLE IF NOT EXISTS `oc_pizza`.`ingredient_quantity` (
  ...
  `ingredient` VARCHAR(50) NOT NULL,
  `quantity` FLOAT NOT NULL,
  `quantity_type` ENUM('gram', 'millilitre', 'tablespoon') NOT NULL,
...
-- -----------------------------------------------------
-- Table `oc_pizza`.`assoc_order_article`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `oc_pizza`.`assoc_order_article` (
  `order_id` INT NOT NULL,
  `article` VARCHAR(50) NOT NULL,
  PRIMARY KEY (`order_id`, `article`),
  INDEX `article_idx` (`article` ASC),
  CONSTRAINT `FK_assoc_order_article_order_id`
    FOREIGN KEY (`order_id`)
    REFERENCES `oc_pizza`.`order` (`order_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_assoc_order_article_article`
    FOREIGN KEY (`article`)
